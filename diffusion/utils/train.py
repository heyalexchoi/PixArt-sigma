...
def clamp_grad_norm_(
        parameters: _tensor_or_tensors, min_norm: float, max_norm: float, norm_type: float = 2.0,
        error_if_nonfinite: bool = False, foreach: Optional[bool] = None) -> torch.Tensor:
    r"""Ensure the gradient norm of an iterable of parameters is at least min_norm and at most max_norm.

    The norm is computed over all gradients together, as if they were
    concatenated into a single vector. Gradients are modified in-place.

    Adapted from torch.nn.utils.clip_grad_norm_ to allow for clamping the gradient norm to a minimum value.

    Args:
        parameters (Iterable[Tensor] or Tensor): an iterable of Tensors or a
            single Tensor that will have gradients normalized
        min_norm (float): min norm of the gradients
    ...
    if error_if_nonfinite and torch.logical_or(total_norm.isnan(), total_norm.isinf()):
        raise RuntimeError(
            ...
            '`parameters` is non-finite, so it cannot be scaled. To disable '
            'this error and scale the gradients by the non-finite norm anyway, '
            'set `error_if_nonfinite=False`')
    
    # Calculate the scaling coefficient
    # put coefficient on device to avoid having to move gradients to cpu to compare
    scale_coef = torch.tensor(1.0, device=first_device)
    if total_norm < min_norm:
        scale_coef = min_norm / (total_norm + 1e-6)
    elif total_norm > max_norm:
        scale_coef = max_norm / (total_norm + 1e-6)
    
    logger.info(f'unclipped grad norm: {total_norm:.4f}, clipping to min: {min_norm:.4f}, max: {max_norm:.4f}, scale_coef: {scale_coef:.4f}')

    if scale_coef != 1.0:  # Only scale if necessary
        for ((device, _), ([grads], _)) in grouped_grads.items():  # type: ignore[assignment]
            if (foreach is None or foreach) and _has_foreach_support(grads, device=device):  # type: ignore[arg-type]
                torch._foreach_mul_(grads, scale_coef.to(device))  # type: ignore[call-overload]
            elif foreach:
                raise RuntimeError(f'foreach=True was passed, but can\'t use the foreach API on {device.type} tensors')
            else:
                scale_coef_device = scale_coef.to(device)
                for g in grads:
                    g.detach().mul_(scale_coef_device)
